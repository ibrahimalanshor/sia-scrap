...
module.exports = async function login(page, data) {
    ...
    const chaptaAnswerInput = await loginForm.$(loginSelectors.chapthaAnswerInput)
    ...
    const chaptaText = await page.evaluate(el => extractMath(el.textContent), chaptaQuestionP)
    const chaptaAnswer = calculator[chaptaText.operator](chaptaText.firstValue, chaptaText.lastValue)
